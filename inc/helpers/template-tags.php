...
function agtheme_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'ag-theme' ),
 ...
 * @return void
 */
function agtheme_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'ag-theme' ),
 ...
 * @return bool|string
 */
function agtheme_the_excerpt( $trim_character_count = 0 ) {
...
function agtheme_pagination() {
	...
	printf( '<nav class="agtheme-pagination">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}